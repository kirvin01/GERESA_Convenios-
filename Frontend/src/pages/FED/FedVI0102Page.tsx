// src/components/FedVI0102Page.tsx
...
import {
  ...
  type ResumenRow,
  type UnidadEjecutoraRow,
} from '../../services/servicesFED/fedVI0102Service';
...
export function FedVI0102Page() {
  ...
  const [unidadEjecutora, setUnidadEjecutora] = useState<string>('');
  ...
  const [unidadesResumen, setUnidadesResumen] = useState<UnidadEjecutoraRow[]>([]);
  ...
  useEffect(() => {
    ...
    fedVI0102Service.getFiltros(ctrl.signal)
      .then((data) => {
        setFiltros(data);
  ...
  // ── Cargar resumen de unidades ejecutoras ──
  useEffect(() => {
    if (!anio) return;
    const ctrl = new AbortController();
    fedVI0102Service.getResumenUnidadesEjecutoras(
      { anio, departamento: departamento || undefined },
      ctrl.signal,
    )
      .then((r) => setUnidadesResumen(r.data))
      .catch((e: Error) => { if (e.name !== 'AbortError') setUnidadesResumen([]); });
    return () => ctrl.abort();
  }, [anio, departamento]);
  ...
  useEffect(() => {
    ...
    return () => ctrl.abort();
  }, [anio, departamento, red, unidadEjecutora, modoVista]);
  ...
  const cargarTablaCompleta = useCallback((signal?: AbortSignal) => {
    ...
    fedVI0102Service.getTablaCompleta(
      { anio, mes, departamento: departamento || undefined, provincia: provincia || undefined, unidad_ejecutora: unidadEjecutora || undefined },
      signal,
    )
      .then((data) => {
        setTabla(data);
  ...
  }, [anio, mes, departamento, provincia, unidadEjecutora, modoVista]);
  ...
  const cargarTablaRedes = useCallback((signal?: AbortSignal) => {
    ...
    setLoadingTabla(true);
    setNotification(null);
    fedVI0102Service.getTablaRedes(
      { anio, mes, red: red || undefined, microred: microred || undefined, unidad_ejecutora: unidadEjecutora || undefined },
      signal,
    )
      .then((data) => {
        setTablaRedes(data);
      ...
      .finally(() => setLoadingTabla(false));
  }, [anio, mes, red, microred, unidadEjecutora, modoVista]);
  ...
  return (
    <Container maxWidth="xl" sx={{ mt: 3, mb: 4 }}>
      ...
      <Box display="flex" alignItems="center" gap={1.5} mb={1}>
        ...
        <Box>
          <Typography variant="h4" fontWeight={700}>FED VI-01.02</Typography>
          <Typography color="text.secondary">
            Porcentaje de gestantes atendidas en establecimientos de salud del Gobierno Regional, que reciben un paquete mínimo de intervenciones terapéuticas especializadas.
          </Typography>
        </Box>
      </Box>
      <Divider sx={{ mb: 3 }} />
