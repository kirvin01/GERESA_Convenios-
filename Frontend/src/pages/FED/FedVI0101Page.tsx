// src/components/FedVI0101Page.tsx
...
} from '../../services/servicesFED/fedVI0101Service';
...
// ── Columnas para el listado nominal ──────────────────────────────────────────

const columnasNominal: GridColDef[] = [
  ...
  { field: 'DISTRITO',        headerName: 'Distrito',        width: 140, sortable: false },
  { field: 'ESTABLECIMIENTO', headerName: 'Establecimiento', width: 160, sortable: false },
  { field: 'num_doc',         headerName: 'N° Documento',    width: 130, sortable: false },
  { field: 'cod_item',        headerName: 'Código Item',     width: 110, sortable: false },
  { field: 'fecha_aten',      headerName: 'Fecha Atención',  width: 110, sortable: false,
    renderCell: (p) => formatDate(p.value as string) },
  { field: 'denominador',     headerName: 'Denominador',     width: 100, type: 'number', headerAlign: 'center', align: 'center' },
  { field: 'numerador',       headerName: 'Numerador',       width: 100, type: 'number', headerAlign: 'center', align: 'center' },
];
...
function GraficoBarras({
  titulo, data, colorScheme = 'primary',
}: {
  titulo: string;
  ...
  const barColor = colorScheme === 'primary' ? '#1565C0' : '#00695C';
  const lightBarColor = alpha(barColor, 0.6);
  ...
  return (
    <Paper
      ...
      sx={{
        ...
        border: '1px solid', borderColor: alpha(barColor, 0.12),
        boxShadow: `0 4px 24px ${alpha(barColor, 0.08)}`,
      }}
    >
      ...
      <ResponsiveContainer width="100%" height={210}>
        <BarChart data={data} margin={{ top: 4, right: 8, left: -16, bottom: 4 }}>
          ...
          <Bar dataKey="Denominador" fill={lightBarColor} radius={[4, 4, 0, 0]} />
          <Bar dataKey="Numerador" fill={barColor} radius={[4, 4, 0, 0]} />
        </BarChart>
      </ResponsiveContainer>
    </Paper>
  );
...
// ── Modal de listado nominal ──────────────────────────────────────────────────

function NominalModal({
  open, onClose, anio, mes, filtros,
}: {
  open: boolean;
  onClose: () => void;
  anio: number;
  ...
  const [data, setData] = useState<NominalRow[]>([]);
  ...
  const cargarNominal = useCallback(async () => {
    setLoading(true);
    try {
      const response = await fedVI0101Service.getNominal({
        anio,
        mes,
        ...filtros,
        page,
      ...
      console.error('Error cargando nominal:', error);
    } finally {
      setLoading(false);
    }
  }, [anio, mes, filtros, page]);
              ...
              <Table stickyHeader size="small">
                <TableHead>
                  <TableRow>
                    ...
                    <TableCell align="center">Denom.</TableCell>
                ...
                <TableBody>
                  {data.map((row, idx) => (
                    <TableRow key={idx} hover>
                      ...
                      <TableCell>{row.num_doc}</TableCell>
                      <TableCell>{row.cod_item}</TableCell>
                      <TableCell>{formatDate(row.fecha_aten)}</TableCell>
                      <TableCell align="center">{row.denominador}</TableCell>
                      <TableCell align="center">{row.numerador}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
...
export function FedVI0101Page() {
  ...
  const [nominalFiltros, setNominalFiltros] = useState<{
    departamento?: string;
    provincia?: string;
    red?: string;
    microred?: string;
  ...
  useEffect(() => {
    ...
    fedVI0101Service.getFiltros(ctrl.signal)
      .then((data) => {
        setFiltros(data);
  ...
  const cargarTablaCompleta = useCallback((signal?: AbortSignal) => {
    ...
    fedVI0101Service.getTablaCompleta(
      { anio, mes, departamento: departamento || undefined, provincia: provincia || undefined, red: red || undefined, microred: microred || undefined, categoria: categoria || undefined },
      signal,
    )
      .then((data) => {
        setTabla(data);
  ...
  }, [anio, mes, departamento, provincia, red, microred, categoria, modoVista]);
  ...
  const cargarTablaRedes = useCallback((signal?: AbortSignal) => {
    ...
    setLoadingTabla(true);
    setNotification(null);
    fedVI0101Service.getTablaRedes(
      { anio, mes, red: red || undefined, microred: microred || undefined, departamento: departamento || undefined, provincia: provincia || undefined, categoria: categoria || undefined },
      signal,
    )
      .then((data) => {
        setTablaRedes(data);
      ...
      .finally(() => setLoadingTabla(false));
  }, [anio, mes, red, microred, departamento, provincia, categoria, modoVista]);
  ...
  // ── Abrir modal con filtros actuales ──
  const abrirNominal = () => {
    setNominalFiltros({
      departamento: departamento || undefined,
      provincia: provincia || undefined,
      red: red || undefined,
      microred: microred || undefined,
  ...
  return (
    <Container maxWidth="xl" sx={{ mt: 3, mb: 4 }}>
      ...
      <Box display="flex" alignItems="center" gap={1.5} mb={1}>
        ...
        <Box>
          <Typography variant="h4" fontWeight={700}>FED VI-01.01</Typography>
          <Typography color="text.secondary">
            Porcentaje de gestantes tamizadas para violencia basada en género
          </Typography>
        </Box>
      </Box>
      <Divider sx={{ mb: 3 }} />
        ...
        <Grid container spacing={2} mb={3}>
          ...
          <Grid item xs={12} sm={8} md={5}>
            <GraficoBarras
              ...
              data={datosGrafico}
              colorScheme={modoVista === 'territorial' ? 'primary' : 'success'}
            />
          </Grid>
      ...
      {!mes ? (
        ...
        >
          <ShieldIcon sx={{ fontSize: 52, color: alpha('#1565C0', 0.2), mb: 1 }} />
