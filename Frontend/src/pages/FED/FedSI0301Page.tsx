// src/components/FedSI0301Page.tsx
...
} from '../../services/servicesFED/fedSI0301Service';
...
const columnasNominal: GridColDef[] = [
  ...
  { field: 'edad',            headerName: 'Edad',            width: 70, type: 'number', headerAlign: 'center', align: 'center' },
  { field: 'fecha_atendida',  headerName: 'Fecha Atención',  width: 110, sortable: false,
...
function NominalModal({
  ...
  const cargarNominal = useCallback(async () => {
    setLoading(true);
    try {
      const response = await fedSI0301Service.getTablaNominal({
        anio,
        mes,
        ...filtros,
        skip,
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="xl" fullWidth>
      <DialogTitle sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
        ...
        Listado Nominal - Gestantes Adolescentes (10-19 años)
        <Typography variant="caption" color="text.secondary" sx={{ ml: 2 }}>
          {mes} {anio} · {total} registros
        </Typography>
      </DialogTitle>
                ...
                <TableBody>
                  {data.map((row, idx) => (
                    <TableRow key={idx} hover>
                      ...
                      <TableCell>{formatDate(row.fecha_atendida)}</TableCell>
...
export function FedSI0301Page() {
  ...
  useEffect(() => {
    ...
    fedSI0301Service.getFiltros(ctrl.signal)
      .then((data) => {
        setFiltros(data);
  ...
  const cargarTablaCompleta = useCallback((signal?: AbortSignal) => {
    ...
    fedSI0301Service.getTablaCompleta(
  ...
  const cargarTablaRedes = useCallback((signal?: AbortSignal) => {
    ...
    setLoadingTabla(true);
    setNotification(null);
    fedSI0301Service.getTablaRedes(
  ...
  return (
    <Container maxWidth="xl" sx={{ mt: 3, mb: 4 }}>
      ...
      <Box display="flex" alignItems="center" gap={1.5} mb={1}>
        ...
        <Box>
          <Typography variant="h4" fontWeight={700}>FED SI-03.01</Typography>
          <Typography color="text.secondary">
            Condición previa: Porcentaje de adolescentes mujeres de 12 a 17 años de edad, atendidas en establecimientos de salud del Gobierno Regional, con dosaje de hemoglobina.
          </Typography>
        </Box>
      </Box>
      <Divider sx={{ mb: 3 }} />
      ...
      {!mes ? (
        ...
        >
          <SchoolIcon sx={{ fontSize: 52, color: alpha('#1565C0', 0.2), mb: 1 }} />
