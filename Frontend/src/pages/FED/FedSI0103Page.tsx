// src/components/FedSI0103Page.tsx
// Reporte FED SI-01_03 — Gestantes: Hb1 + suplementación hierro (5 dosis) + Hb2/Hb3
...
import {
  ...
  Hub as HubIcon,
  Favorite as BloodtypeIcon,
  LocalPharmacy as VaccinesIcon,
  Assessment as MonitorHeartIcon,
...
import {
  ...
  type ResumenRow,
} from '../../services/servicesFED/fedSI0103Service';

// ── Paleta de colores específica para SI-01_03 ────────────────────────────────
// Morado/violeta — diferencia visual clara del MC-01.01 (azul) y otros reportes
const COLOR_TERRITORIAL = '#1565C0';   // purple darken-3
const COLOR_REDES       = '#00695C';   // teal darken-3
const COLOR_HB          = '#AD1457';   // pink darken-3
const COLOR_SUPL        = '#E65100';   // deep-orange darken-3
...
function MetricaChip({
  ...
  return (
    <Tooltip title={`${fmtNum(value)} de ${fmtNum(total)}`} arrow>
      <Box
        sx={{
      ...
      >
        <Typography variant="caption" fontWeight={600} color="text.secondary"
          sx={{ fontSize: '0.68rem', lineHeight: 1.2, textAlign: 'center', mb: 0.4 }}>
          {label}
        </Typography>
        <Typography variant="body2" fontWeight={800} sx={{ color, lineHeight: 1 }}>
          {pct === '—' ? '—' : `${pct}%`}
        </Typography>
...
// ── Panel de métricas clínicas (Hb1, supl, Hb2, Hb3) ─────────────────────────
{/*}
function PanelMetricasClinicas({ total }: { total: TotalRow }) {
  const denom = total.denominador ?? 0;
  return (
    <Paper
      elevation={0}
      sx={{
        p: 2, borderRadius: 3, mb: 3,
        border: '1px solid', borderColor: alpha(COLOR_HB, 0.18),
        boxShadow: `0 4px 24px ${alpha(COLOR_HB, 0.07)}`,
      }}
    >
      <Typography variant="subtitle2" fontWeight={700} color="text.secondary" mb={1.5}
        sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
        <MonitorHeartIcon fontSize="small" sx={{ color: COLOR_HB }} />
        Seguimiento Clínico — Cobertura de Indicadores
      </Typography>

      {/* Hb1 *
      <Box mb={1.5}>
        <Typography variant="caption" fontWeight={700} color="text.disabled"
          sx={{ display: 'flex', alignItems: 'center', gap: 0.4, mb: 0.75 }}>
          <BloodtypeIcon sx={{ fontSize: 14, color: COLOR_HB }} />
          Hemoglobina
        </Typography>
        <Box display="flex" gap={1} flexWrap="wrap">
          <MetricaChip
            label="Hb1 Sem.14" value={total.total_denom_Hb1 ?? 0}
            total={denom} color={COLOR_HB} />
          <MetricaChip
            label="Dx Hb1" value={total.total_denom_Hb1_Dx ?? 0}
            total={denom} color={COLOR_HB} />
          <MetricaChip
            label="Hb2" value={total.total_num_Hb2 ?? 0}
            total={denom} color={COLOR_HB} />
          <MetricaChip
            label="Hb3" value={total.total_num_Hb3 ?? 0}
            total={denom} color={COLOR_HB} />
        </Box>
      </Box>
      ...
      <Box>
        <Typography variant="caption" fontWeight={700} color="text.disabled"
          sx={{ display: 'flex', alignItems: 'center', gap: 0.4, mb: 0.75 }}>
          <VaccinesIcon sx={{ fontSize: 14, color: COLOR_SUPL }} />
          Suplementación de Hierro (5 dosis)
        </Typography>
        <Box display="flex" gap={1} flexWrap="wrap">
          {[
            { label: 'Dosis 1', val: total.total_num_supl1 ?? 0 },
            { label: 'Dosis 2', val: total.total_num_supl2 ?? 0 },
            { label: 'Dosis 3', val: total.total_num_supl3 ?? 0 },
            { label: 'Dosis 4', val: total.total_num_supl4 ?? 0 },
            { label: 'Dosis 5', val: total.total_num_supl5 ?? 0 },
          ].map(({ label, val }) => (
            <MetricaChip
              key={label}
              label={label}
...
const columnasDetalleTerritorial: GridColDef[] = [
  ...
  { field: 'total_denom_Hb1',    headerName: 'Hb1',    width: 80,  type: 'number', headerAlign: 'center', align: 'center' },
  { field: 'total_denom_Hb1_Dx', headerName: 'Dx Hb1', width: 80,  type: 'number', headerAlign: 'center', align: 'center' },
  { field: 'total_num_supl1',    headerName: 'Supl.1', width: 75,  type: 'number', headerAlign: 'center', align: 'center' },
  { field: 'total_num_supl2',    headerName: 'Supl.2', width: 75,  type: 'number', headerAlign: 'center', align: 'center' },
  { field: 'total_num_supl3',    headerName: 'Supl.3', width: 75,  type: 'number', headerAlign: 'center', align: 'center' },
  { field: 'total_num_supl4',    headerName: 'Supl.4', width: 75,  type: 'number', headerAlign: 'center', align: 'center' },
  { field: 'total_num_supl5',    headerName: 'Supl.5', width: 75,  type: 'number', headerAlign: 'center', align: 'center' },
  { field: 'total_num_Hb2',      headerName: 'Hb2',    width: 80,  type: 'number', headerAlign: 'center', align: 'center' },
  { field: 'total_num_Hb3',      headerName: 'Hb3',    width: 80,  type: 'number', headerAlign: 'center', align: 'center' },
];
...
const columnasDetalleRedes: GridColDef[] = [
  ...
  { field: 'MICRORED',        headerName: 'Microred',        width: 150, sortable: false },
  { field: 'ESTABLECIMIENTO', headerName: 'Establecimiento', flex: 1, minWidth: 150, sortable: false },
  { field: 'denominador',     headerName: 'Denominador',     width: 120, type: 'number', headerAlign: 'center', align: 'center' },
  { field: 'numerador',       headerName: 'Numerador',       width: 110, type: 'number', headerAlign: 'center', align: 'center' },
  {
    field: 'avance_pct', headerName: 'Avance %', width: 120,
    ...
    <>
      <Box
        ...
        sx={{
          ...
          bgcolor: alpha(COLOR_TERRITORIAL, 0.10), cursor: 'pointer',
          borderBottom: '1px solid', borderColor: alpha(COLOR_TERRITORIAL, 0.15),
          '&:hover': { bgcolor: alpha(COLOR_TERRITORIAL, 0.15) },
        }}
      >
        <IconButton size="small" sx={{ p: 0 }}>
          {open ? <ArrowDownIcon fontSize="small" /> : <ArrowRightIcon fontSize="small" />}
        </IconButton>
        <Typography variant="body2" fontWeight={700} sx={{ color: COLOR_TERRITORIAL }}>{prov.DEPARTAMENTO}</Typography>
        <Typography variant="body2" fontWeight={700} sx={{ color: COLOR_TERRITORIAL }}>Total {prov.PROVINCIA}</Typography>
        ...
        {filas.map((d) => (
          <Box
            ...
            sx={{
              ...
              '&:hover': { bgcolor: alpha(COLOR_TERRITORIAL, 0.04) },
            }}
          >
            <Box />
    ...
    <>
      <Box
        ...
        sx={{
          ...
          bgcolor: alpha(COLOR_REDES, 0.05), cursor: 'pointer',
          borderBottom: '1px solid', borderColor: alpha(COLOR_REDES, 0.10),
          '&:hover': { bgcolor: alpha(COLOR_REDES, 0.10) },
        }}
      >
        <Box />
        <IconButton size="small" sx={{ p: 0 }}>
        ...
        <Typography variant="body2" fontWeight={600} sx={{ color: COLOR_REDES }}>
          {microred.MICRORED}
        </Typography>
        ...
        {filas.map((e) => (
          <Box
            ...
            sx={{
              ...
              borderBottom: '1px solid', borderColor: alpha('#000', 0.04),
              '&:hover': { bgcolor: alpha(COLOR_REDES, 0.04) },
            }}
          >
            <Box /><Box />
    ...
    <>
      <Box
        ...
        sx={{
          ...
          gridTemplateColumns: '40px 1fr 1fr 110px 100px 120px',
          alignItems: 'center', px: 1, py: 0.75,
          bgcolor: alpha(COLOR_REDES, 0.10), cursor: 'pointer',
          borderBottom: '1px solid', borderColor: alpha(COLOR_REDES, 0.15),
          '&:hover': { bgcolor: alpha(COLOR_REDES, 0.15) },
        }}
      >
        <IconButton size="small" sx={{ p: 0 }}>
          {open ? <ArrowDownIcon fontSize="small" /> : <ArrowRightIcon fontSize="small" />}
        </IconButton>
        <Typography variant="body2" fontWeight={700} sx={{ color: COLOR_REDES }}>
          {red.RED}
        </Typography>
        <Typography variant="body2" fontWeight={700} sx={{ color: COLOR_REDES }}>
          Total Red
  ...
  return (
    <Paper
      ...
      sx={{
        ...
        border: '1px solid', borderColor: alpha(colorPrimary, 0.12),
        boxShadow: `0 4px 24px ${alpha(colorPrimary, 0.08)}`,
      }}
    >
      <Typography variant="subtitle2" fontWeight={700} color="text.secondary" mb={1.5}>
        {titulo}
      ...
      <ResponsiveContainer width="100%" height={210}>
        <BarChart data={data} margin={{ top: 4, right: 8, left: -16, bottom: 4 }}>
          ...
          <Bar dataKey="Denominador" fill={alpha(colorPrimary, 0.4)} radius={[4, 4, 0, 0]} />
          <Bar dataKey="Numerador"   fill={colorPrimary}             radius={[4, 4, 0, 0]} />
        </BarChart>
      </ResponsiveContainer>
    </Paper>
  );
...
function GraficoTendencia({ resumen }: { resumen: ResumenRow[] }) {
  ...
  return (
    <Paper
      ...
      sx={{
        p: 2, borderRadius: 3,
        border: '1px solid', borderColor: alpha(COLOR_TERRITORIAL, 0.12),
        boxShadow: `0 4px 24px ${alpha(COLOR_TERRITORIAL, 0.08)}`,
      }}
    >
      <Typography variant="subtitle2" fontWeight={700} color="text.secondary" mb={1.5}>
      ...
      <ResponsiveContainer width="100%" height={180}>
        <BarChart data={data} margin={{ top: 4, right: 8, left: -16, bottom: 4 }}>
          ...
          />
          <Bar dataKey="Avance %" fill="#00695C" radius={[4, 4, 0, 0]} />
        </BarChart>
      </ResponsiveContainer>
    </Paper>
  );
...
export function FedSI0103Page() {
  ...
  useEffect(() => {
    ...
    fedSI0103Service.getFiltros(ctrl.signal)
      .then((data) => {
        setFiltros(data);
  ...
  const cargarTablaCompleta = useCallback((signal?: AbortSignal) => {
    ...
    fedSI0103Service.getTablaCompleta(
  ...
  const cargarTablaRedes = useCallback((signal?: AbortSignal) => {
    ...
    setLoadingTabla(true);
    setNotification(null);
    fedSI0103Service.getTablaRedes(
  ...
  const colorActual  = modoVista === 'territorial' ? COLOR_TERRITORIAL : COLOR_REDES;
  ...
  return (
    <Container maxWidth="xl" sx={{ mt: 3, mb: 4 }}>
      ...
      <Box display="flex" alignItems="center" gap={1.5} mb={1}>
        <Avatar sx={{ bgcolor: alpha(COLOR_TERRITORIAL, 0.1), color: COLOR_TERRITORIAL, width: 44, height: 44 }}>
          <PregnantWomanIcon />
        </Avatar>
        <Box>
          <Typography variant="h4" fontWeight={700}>FED SI-01.03</Typography>
          <Typography color="text.secondary">
            Porcentaje de mujeres con parto institucional sin anemia atendidas en establecimientos de salud del Gobierno Regional, que durante su gestación recibieron suplementación preventiva con hierro y dosajes de hemoglobina.
          </Typography>
        </Box>
      </Box>
      <Divider sx={{ mb: 3 }} />
      ...
      <Box display="flex" justifyContent="center" mb={3}>
        <ToggleButtonGroup
          ...
          sx={{
            ...
            border: '1px solid',
            borderColor: alpha(COLOR_TERRITORIAL, 0.20),
            borderRadius: 3,
            overflow: 'hidden',
            boxShadow: `0 2px 12px ${alpha(COLOR_TERRITORIAL, 0.10)}`,

            '& .MuiToggleButton-root': {
              px: 3, py: 1.2, fontWeight: 600, fontSize: '0.88rem',
              textTransform: 'none', border: 'none', gap: 0.8, color: 'text.secondary',
              '&:hover': { bgcolor: alpha(COLOR_TERRITORIAL, 0.06) },
            },

            '& .MuiToggleButton-root[value="territorial"].Mui-selected': {
              bgcolor: COLOR_TERRITORIAL, color: '#fff',
              '&:hover': { bgcolor: COLOR_TERRITORIAL },
            },

            '& .MuiToggleButton-root[value="redes"].Mui-selected': {
              bgcolor: COLOR_REDES, color: '#fff',
              '&:hover': { bgcolor: COLOR_REDES },
            },
          }}
        >
      ...
      <Paper
        ...
        sx={{
          ...
          border: '1px solid', borderColor: alpha(colorActual, 0.12),
          boxShadow: `0 4px 24px ${alpha(colorActual, 0.08)}`,
        }}
      >
        <Typography
          ...
          <Grid item xs={12} sm={4} md={3}>
            <Paper
              ...
              sx={{
                ...
                background: `linear-gradient(135deg, ${colorActual} 0%, ${alpha(colorActual, 0.75)} 100%)`,
      ...
      {/* ── Panel de métricas clínicas SI-01_03 ── 
      {totalActual && <PanelMetricasClinicas total={totalActual} />} */}
      ...
      {!mes ? (
        <Paper
          ...
          sx={{
            ...
            border: '2px dashed', borderColor: alpha(COLOR_TERRITORIAL, 0.2),
            color: 'text.secondary',
          }}
        >
          <PregnantWomanIcon sx={{ fontSize: 52, color: alpha(COLOR_TERRITORIAL, 0.2), mb: 1 }} />
        ...
        >
          ...
          <Box
            sx={{
              px: 2.5, py: 1.5,
              bgcolor: alpha(colorActual, 0.03),
              borderBottom: '1px solid', borderColor: alpha(colorActual, 0.10),
          ...
          >
            <Box display="flex" alignItems="center" gap={1}>
              ...
              <Chip
                ...
                size="small"
                sx={{
                  bgcolor: alpha(colorActual, 0.12),
                  color: colorActual,
                  fontWeight: 700,
                  border: `1px solid ${alpha(colorActual, 0.25)}`,
                }}
              />
            </Box>
                ...
                <>
                  <Box
                    sx={{
                      ...
                      gridTemplateColumns: '40px 1fr 1fr 110px 100px 120px',
                      px: 1, py: 1,
                      bgcolor: alpha(COLOR_TERRITORIAL, 0.06),
                      borderBottom: '2px solid', borderColor: alpha(COLOR_TERRITORIAL, 0.15),
                    }}
                  >
                  ...
                  {tabla.total && (
                    <Box
                      sx={{
                        ...
                        bgcolor: COLOR_TERRITORIAL, borderTop: '2px solid', borderColor: alpha(COLOR_TERRITORIAL, 0.8),
                      }}
                    >
                      <Box />
              ...
              {vista === 'plana' && (
                <Box sx={{ height: 560 }}>
                  <DataGrid
                    ...
                    sx={{
                      border: 'none',
                      '& .MuiDataGrid-columnHeaders': { bgcolor: alpha(COLOR_TERRITORIAL, 0.05), borderBottom: `2px solid ${alpha(COLOR_TERRITORIAL, 0.15)}` },
                      '& .MuiDataGrid-columnHeaderTitle': { fontWeight: 700, fontSize: '0.8rem', color: '#1A2332' },
                      '& .MuiDataGrid-row:hover': { bgcolor: alpha(COLOR_TERRITORIAL, 0.04) },
                      '& .MuiDataGrid-row:nth-of-type(even)': { bgcolor: alpha(COLOR_TERRITORIAL, 0.015) },
                      '& .MuiDataGrid-cell': { borderColor: alpha('#000', 0.05) },
                      '& .MuiDataGrid-footerContainer': { borderTop: `1px solid ${alpha('#000', 0.08)}`, bgcolor: alpha(COLOR_TERRITORIAL, 0.02) },
                    }}
                  />
                </Box>
              )}
                ...
                <>
                  <Box
                    sx={{
                      ...
                      gridTemplateColumns: '40px 1fr 1fr 110px 100px 120px',
                      px: 1, py: 1,
                      bgcolor: alpha(COLOR_REDES, 0.06),
                      borderBottom: '2px solid', borderColor: alpha(COLOR_REDES, 0.15),
                    }}
                  >
                  ...
                  {tablaRedes.total && (
                    <Box
                      sx={{
                        ...
                        gridTemplateColumns: '40px 1fr 1fr 110px 100px 120px',
                        alignItems: 'center', px: 1, py: 1,
                        bgcolor: COLOR_REDES, borderTop: '2px solid', borderColor: alpha(COLOR_REDES, 0.8),
                      }}
                    >
                      <Box />
                      <Typography variant="body2" fontWeight={900} color="#fff">TOTAL GENERAL</Typography>
              ...
              {vista === 'plana' && (
                <Box sx={{ height: 560 }}>
                  <DataGrid
                    ...
                    sx={{
                      border: 'none',
                      '& .MuiDataGrid-columnHeaders': { bgcolor: alpha(COLOR_REDES, 0.05), borderBottom: `2px solid ${alpha(COLOR_REDES, 0.15)}` },
                      '& .MuiDataGrid-columnHeaderTitle': { fontWeight: 700, fontSize: '0.8rem', color: '#1A2332' },
                      '& .MuiDataGrid-row:hover': { bgcolor: alpha(COLOR_REDES, 0.04) },
                      '& .MuiDataGrid-row:nth-of-type(even)': { bgcolor: alpha(COLOR_REDES, 0.015) },
                      '& .MuiDataGrid-cell': { borderColor: alpha('#000', 0.05) },
                      '& .MuiDataGrid-footerContainer': { borderTop: `1px solid ${alpha('#000', 0.08)}`, bgcolor: alpha(COLOR_REDES, 0.02) },
                    }}
                  />
                </Box>
              )}
