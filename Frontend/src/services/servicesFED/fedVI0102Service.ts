// src/services/fedVI0102Service.ts
// Servicio para el reporte FED VI-01.02 — Nominales de gestantes

import { API_CONFIG } from '../../config';
import { authHeader } from '../authService';

const BASE = `${API_CONFIG.baseURL}/fed/vi0102`;
...
export interface UnidadEjecutoraRow {
...
export interface TablaRedesData {
  ...
  mes:                 string;
  total:               TotalRow;
  unidades_ejecutoras: UnidadEjecutoraRow[];
  redes:               RedRow[];
...
export const fedVI0102Service = {
  ...
  getResumenUnidadesEjecutoras: (
    params: { anio: number; departamento?: string },
    signal?: AbortSignal,
  ) => {
    const qs = new URLSearchParams();
    Object.entries(params).forEach(([k, v]) => { if (v) qs.set(k, String(v)); });
    return apiFetch<{ data: UnidadEjecutoraRow[] }>(`${BASE}/resumen-unidades-ejecutoras?${qs}`, signal);
  },

  getResumen: (
    params: { anio?: number; departamento?: string; red?: string; unidad_ejecutora?: string },
    signal?: AbortSignal,
  ) => {
    const qs = new URLSearchParams();
    Object.entries(params).forEach(([k, v]) => { if (v) qs.set(k, String(v)); });
