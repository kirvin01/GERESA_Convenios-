# fed_si_03_02.py
# Router para el reporte FED SI-03.02 — Gestantes con suplementación y consejería
# Base de datos: DBFED2026 | Tabla: IRVIN_FED_SI_03_02
...
router = APIRouter(prefix="/fed/si0302", tags=["FED SI-03.02"])
...
def get_filtros(
    ...
    try:
        anios = db.execute(text(
            "SELECT DISTINCT año FROM DBFED2026.dbo.IRVIN_FED_SI_03_02 ORDER BY año"
        )).fetchall()

        meses = db.execute(text(f"""
            SELECT DISTINCT MES, {MES_ORDER} AS orden
            FROM DBFED2026.dbo.IRVIN_FED_SI_03_02 ORDER BY orden
        """)).fetchall()

        departamentos = db.execute(text(
            "SELECT DISTINCT DEPARTAMENTO FROM DBFED2026.dbo.IRVIN_FED_SI_03_02 ORDER BY DEPARTAMENTO"
        )).fetchall()

        provincias = db.execute(text(
            "SELECT DISTINCT DEPARTAMENTO, PROVINCIA FROM DBFED2026.dbo.IRVIN_FED_SI_03_02 ORDER BY DEPARTAMENTO, PROVINCIA"
        )).fetchall()

        redes = db.execute(text(
            "SELECT DISTINCT RED FROM DBFED2026.dbo.IRVIN_FED_SI_03_02 WHERE RED IS NOT NULL ORDER BY RED"
        )).fetchall()

        microredes = db.execute(text(
            "SELECT DISTINCT RED, MICRORED FROM DBFED2026.dbo.IRVIN_FED_SI_03_02 WHERE MICRORED IS NOT NULL ORDER BY RED, MICRORED"
        )).fetchall()

        categorias = db.execute(text(
            "SELECT DISTINCT CATEGORIA FROM DBFED2026.dbo.IRVIN_FED_SI_03_02 WHERE CATEGORIA IS NOT NULL ORDER BY CATEGORIA"
        )).fetchall()
...
# ---------------------------------------------------------------------------
# GET /fed/si0302/tabla-completa — DEPARTAMENTO → PROVINCIA → DISTRITO
# ---------------------------------------------------------------------------
...
def get_tabla_completa(
    ...
    def agg(group_cols, order_cols=None):
        ...
        return text(f"""
            ...
            FROM DBFED2026.dbo.IRVIN_FED_SI_03_02 {w}
...
# ---------------------------------------------------------------------------
# GET /fed/si0302/tabla-redes — RED → MICRORED → ESTABLECIMIENTO
# ---------------------------------------------------------------------------
