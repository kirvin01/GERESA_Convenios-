# fed_si_02_03.py
# Router para el reporte FED SI-02_03 — Suplementación con Hierro y Dosaje de Hemoglobina
# Base de datos: DBFED2026 | Tabla: IRVIN_FED_SI_02_03
...
router = APIRouter(prefix="/fed/si0203", tags=["FED SI-02_03"])
...
# Columnas métricas del reporte SI-02_03
HB_COLS = """
    ...
    CASE WHEN SUM(denominador)>0
         ...
         ELSE 0
    END                          AS avance_pct,
    SUM(denominador_Hb6m)        AS num_Hb6m,
    SUM(denominador_Hb6m_Dx)     AS num_Hb6m_Dx,
    SUM(numerador_Hierro_Trat1)  AS num_Hierro_Trat1,
    SUM(numerador_Hierro_Trat2)  AS num_Hierro_Trat2,
    SUM(numerador_Hierro_Trat3)  AS num_Hierro_Trat3,
    SUM(numerador_Hb7m)          AS num_Hb7m,
    SUM(numerador_Hb8m)          AS num_Hb8m,
...
# ---------------------------------------------------------------------------
# GET /fed/si0203/filtros
# ---------------------------------------------------------------------------
@router.get("/filtros", summary="Valores disponibles para filtros del reporte FED SI-02_03")
def get_filtros(
    ...
    try:
        anios = db.execute(text(
            "SELECT DISTINCT año FROM DBFED2026.dbo.IRVIN_FED_SI_02_03 ORDER BY año"
        )).fetchall()

        meses = db.execute(text(f"""
            SELECT DISTINCT MES, {MES_ORDER} AS orden
            FROM DBFED2026.dbo.IRVIN_FED_SI_02_03
            ORDER BY orden
        """)).fetchall()

        departamentos = db.execute(text(
            "SELECT DISTINCT DEPARTAMENTO FROM DBFED2026.dbo.IRVIN_FED_SI_02_03 ORDER BY DEPARTAMENTO"
        )).fetchall()

        provincias = db.execute(text(
            "SELECT DISTINCT DEPARTAMENTO, PROVINCIA FROM DBFED2026.dbo.IRVIN_FED_SI_02_03 ORDER BY DEPARTAMENTO, PROVINCIA"
        )).fetchall()

        redes = db.execute(text(
            "SELECT DISTINCT RED FROM DBFED2026.dbo.IRVIN_FED_SI_02_03 WHERE RED IS NOT NULL ORDER BY RED"
        )).fetchall()

        microredes = db.execute(text(
            "SELECT DISTINCT RED, MICRORED FROM DBFED2026.dbo.IRVIN_FED_SI_02_03 WHERE MICRORED IS NOT NULL ORDER BY RED, MICRORED"
        )).fetchall()

        categorias = db.execute(text(
            "SELECT DISTINCT CATEGORIA FROM DBFED2026.dbo.IRVIN_FED_SI_02_03 WHERE CATEGORIA IS NOT NULL ORDER BY CATEGORIA"
        )).fetchall()
...
# ---------------------------------------------------------------------------
# GET /fed/si0203/tabla-completa  — DEPARTAMENTO → PROVINCIA → DISTRITO
# ---------------------------------------------------------------------------
...
def get_tabla_completa(
    ...
    try:
        distritos = db.execute(text(f"""
            SELECT DEPARTAMENTO, PROVINCIA, DISTRITO, {HB_COLS}
            FROM DBFED2026.dbo.IRVIN_FED_SI_02_03 {w}
            GROUP BY DEPARTAMENTO, PROVINCIA, DISTRITO
            ORDER BY DEPARTAMENTO, PROVINCIA, DISTRITO
        """), params).fetchall()

        provincias_rows = db.execute(text(f"""
            SELECT DEPARTAMENTO, PROVINCIA, {HB_COLS}
            FROM DBFED2026.dbo.IRVIN_FED_SI_02_03 {w}
...
# ---------------------------------------------------------------------------
# GET /fed/si0203/tabla-redes  — RED → MICRORED → ESTABLECIMIENTO
# ---------------------------------------------------------------------------
...
def get_tabla_redes(
    ...
    try:
        ...
        microredes_rows = db.execute(text(f"""
            SELECT ISNULL(RED,'SIN RED') AS RED,
                   ISNULL(MICRORED,'SIN MICRORED') AS MICRORED, {HB_COLS}
            FROM DBFED2026.dbo.IRVIN_FED_SI_02_03 {w}
        ...
        redes_rows = db.execute(text(f"""
            SELECT ISNULL(RED,'SIN RED') AS RED, {HB_COLS}
            FROM DBFED2026.dbo.IRVIN_FED_SI_02_03 {w}
            GROUP BY RED
            ORDER BY RED
        """), params).fetchall()
...
# ---------------------------------------------------------------------------
# GET /fed/si0203/resumen
# ---------------------------------------------------------------------------
@router.get("/resumen", summary="Resumen de hierro/Hb por año y mes")
def get_resumen(
        ...
        rows = db.execute(text(f"""
            SELECT año, MES,
                ...
                CASE WHEN SUM(denominador)>0
                     ...
                     ELSE 0
                END                         AS avance_pct,
                SUM(denominador_Hb6m)       AS total_Hb6m,
                SUM(denominador_Hb6m_Dx)    AS total_Hb6m_Dx,
                SUM(numerador_Hierro_Trat1) AS total_Hierro_Trat1,
                SUM(numerador_Hierro_Trat2) AS total_Hierro_Trat2,
                SUM(numerador_Hierro_Trat3) AS total_Hierro_Trat3,
                SUM(numerador_Hb7m)         AS total_Hb7m,
                SUM(numerador_Hb8m)         AS total_Hb8m,
                SUM(numerador_Hb9m)         AS total_Hb9m,
                SUM(numerador_Hb12m)        AS total_Hb12m
            FROM DBFED2026.dbo.IRVIN_FED_SI_02_03 {w}
            GROUP BY año, MES
