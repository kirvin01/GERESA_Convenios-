# fed_si_02_04.py
# Router para el reporte FED SI-02_04
# Niños 6-11 meses con diagnóstico de anemia + tratamiento (SfPo / Multimicronutriente)
# y seguimiento con Hb 9m / 12m
# Base de datos: DBFED2026 | Tabla: IRVIN_FED_SI_02_04
...
router = APIRouter(prefix="/fed/si0204", tags=["FED SI-02_04"])
...
# Columnas de indicadores específicos del reporte SI-02_04
EXTRA_COLS = """
    SUM(numerador_suplementacion_inicio)            AS num_supl_inicio,
    SUM(numerador_suplementacion_continuacion)      AS num_supl_continuacion,
    SUM(numerador_Hb9m)                             AS num_Hb9m,
    SUM(numerador_Hb12m)                            AS num_Hb12m,
    SUM(numerador_suplementacion_SfPo1)             AS num_SfPo1,
    SUM(numerador_suplementacion_SfPo2)             AS num_SfPo2,
    SUM(numerador_suplementacion_Multi1)            AS num_Multi1,
    SUM(numerador_suplementacion_Multi2)            AS num_Multi2,
    SUM(numerador_suplementacion_Multi3)            AS num_Multi3,
    SUM(numerador_suplementacion_Multi4)            AS num_Multi4,
    SUM(numerador_suplementacion_Multi5)            AS num_Multi5,
    SUM(numerador_suplementacion_Multi6)            AS num_Multi6,
    SUM(numerador_suplementacion_multi1_SfPo2)      AS num_multi1_SfPo2,
    SUM(numerador_suplementacion_SfPo1_multi2)      AS num_SfPo1_multi2,
    SUM(numerador_Hb9m_SfPo1)                       AS num_Hb9m_SfPo1,
    SUM(numerador_Hb12m_SfPo1)                      AS num_Hb12m_SfPo1,
    SUM(numerador_Hb9m_multi1)                      AS num_Hb9m_multi1,
    SUM(numerador_Hb12m_multi1)                     AS num_Hb12m_multi1
"""


# ---------------------------------------------------------------------------
# GET /fed/si0204/filtros
# ---------------------------------------------------------------------------
...
def get_filtros(
    ...
    try:
        anios = db.execute(text(
            "SELECT DISTINCT año FROM DBFED2026.dbo.IRVIN_FED_SI_02_04 ORDER BY año"
        )).fetchall()

        meses = db.execute(text(f"""
            SELECT DISTINCT MES, {MES_ORDER} AS orden
            FROM DBFED2026.dbo.IRVIN_FED_SI_02_04 ORDER BY orden
        """)).fetchall()

        departamentos = db.execute(text(
            "SELECT DISTINCT DEPARTAMENTO FROM DBFED2026.dbo.IRVIN_FED_SI_02_04 ORDER BY DEPARTAMENTO"
        )).fetchall()

        provincias = db.execute(text(
            "SELECT DISTINCT DEPARTAMENTO, PROVINCIA FROM DBFED2026.dbo.IRVIN_FED_SI_02_04 ORDER BY DEPARTAMENTO, PROVINCIA"
        )).fetchall()

        redes = db.execute(text(
            "SELECT DISTINCT RED FROM DBFED2026.dbo.IRVIN_FED_SI_02_04 WHERE RED IS NOT NULL ORDER BY RED"
        )).fetchall()

        microredes = db.execute(text(
            "SELECT DISTINCT RED, MICRORED FROM DBFED2026.dbo.IRVIN_FED_SI_02_04 WHERE MICRORED IS NOT NULL ORDER BY RED, MICRORED"
        )).fetchall()

        categorias = db.execute(text(
            "SELECT DISTINCT CATEGORIA FROM DBFED2026.dbo.IRVIN_FED_SI_02_04 WHERE CATEGORIA IS NOT NULL ORDER BY CATEGORIA"
        )).fetchall()
...
# ---------------------------------------------------------------------------
# GET /fed/si0204/tabla-completa  —  DEPARTAMENTO → PROVINCIA → DISTRITO
# ---------------------------------------------------------------------------
    ...
    def agg(group_cols: str):
        return text(f"""
            SELECT {group_cols},
                ...
                {EXTRA_COLS}
            FROM DBFED2026.dbo.IRVIN_FED_SI_02_04 {w}
            GROUP BY {group_cols}
            ORDER BY {group_cols}
        """)
...
# ---------------------------------------------------------------------------
# GET /fed/si0204/tabla-redes  —  RED → MICRORED → ESTABLECIMIENTO
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# GET /fed/si0204/resumen
# ---------------------------------------------------------------------------
@router.get("/resumen", summary="Resumen por año y mes")
def get_resumen(
