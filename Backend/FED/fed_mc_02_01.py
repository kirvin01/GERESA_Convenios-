# fed_mc_02_01.py
# Router para el reporte FED MC-02_01 — Vacunación en niños (CNV)
# Base de datos: DBFED2026 | Tabla: IRVIN_FED_MC_02_01
...
router = APIRouter(prefix="/fed/mc0201", tags=["FED MC-02_01"])
...
# ---------------------------------------------------------------------------
# GET /fed/mc0201/filtros
# ---------------------------------------------------------------------------
@router.get("/filtros", summary="Valores disponibles para filtros del reporte FED MC-02_01")
def get_filtros(
    ...
    try:
        anios = db.execute(text(
            "SELECT DISTINCT año FROM DBFED2026.dbo.IRVIN_FED_MC_02_01 ORDER BY año"
        )).fetchall()

        meses = db.execute(text(f"""
            SELECT DISTINCT MES, {MES_ORDER} AS orden
            FROM DBFED2026.dbo.IRVIN_FED_MC_02_01 ORDER BY orden
        """)).fetchall()

        departamentos = db.execute(text(
            "SELECT DISTINCT DEPARTAMENTO FROM DBFED2026.dbo.IRVIN_FED_MC_02_01 ORDER BY DEPARTAMENTO"
        )).fetchall()

        provincias = db.execute(text(
            "SELECT DISTINCT DEPARTAMENTO, PROVINCIA FROM DBFED2026.dbo.IRVIN_FED_MC_02_01 ORDER BY DEPARTAMENTO, PROVINCIA"
        )).fetchall()

        redes = db.execute(text(
            "SELECT DISTINCT RED FROM DBFED2026.dbo.IRVIN_FED_MC_02_01 WHERE RED IS NOT NULL ORDER BY RED"
        )).fetchall()

        microredes = db.execute(text(
            "SELECT DISTINCT RED, MICRORED FROM DBFED2026.dbo.IRVIN_FED_MC_02_01 WHERE MICRORED IS NOT NULL ORDER BY RED, MICRORED"
        )).fetchall()

        categorias = db.execute(text(
            "SELECT DISTINCT CATEGORIA FROM DBFED2026.dbo.IRVIN_FED_MC_02_01 WHERE CATEGORIA IS NOT NULL ORDER BY CATEGORIA"
        )).fetchall()
...
# ---------------------------------------------------------------------------
# GET /fed/mc0201/tabla-completa  — DEPARTAMENTO → PROVINCIA → DISTRITO
# ---------------------------------------------------------------------------
...
def get_tabla_completa(
    ...
    VAC_COLS = """
        ...
        SUM(num_vac_antineumococica) AS num_vac_antineumococica,
        SUM(num_vac_antipolio)       AS num_vac_antipolio,
        SUM(num_vac_pentavalente)    AS num_vac_pentavalente,
        SUM(num_vac_rotavirus)       AS num_vac_rotavirus,
        SUM(num_esq4M)               AS num_esq4M,
        SUM(num_esq6M)               AS num_esq6M,
        SUM(num_dosaje_Hb)           AS num_dosaje_Hb
    """

    try:
        distritos = db.execute(text(f"""
            SELECT DEPARTAMENTO, PROVINCIA, DISTRITO, {VAC_COLS}
            FROM DBFED2026.dbo.IRVIN_FED_MC_02_01 {w}
            GROUP BY DEPARTAMENTO, PROVINCIA, DISTRITO
            ORDER BY DEPARTAMENTO, PROVINCIA, DISTRITO
        """), params).fetchall()

        provincias = db.execute(text(f"""
            SELECT DEPARTAMENTO, PROVINCIA, {VAC_COLS}
            FROM DBFED2026.dbo.IRVIN_FED_MC_02_01 {w}
...
# ---------------------------------------------------------------------------
# GET /fed/mc0201/tabla-redes  — RED → MICRORED → ESTABLECIMIENTO
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# GET /fed/mc0201/resumen
# ---------------------------------------------------------------------------
@router.get("/resumen", summary="Resumen de vacunación por año y mes")
def get_resumen(
    ...
    try:
        rows = db.execute(text(f"""
            SELECT año, MES,
                ...
                CASE WHEN SUM(denominador)>0 THEN ROUND(CAST(SUM(numerador) AS FLOAT)/SUM(denominador)*100,2) ELSE 0 END AS avance_pct,
                SUM(num_vac_antineumococica) AS total_antineumococica,
                SUM(num_vac_antipolio)       AS total_antipolio,
                SUM(num_vac_pentavalente)    AS total_pentavalente,
                SUM(num_vac_rotavirus)       AS total_rotavirus,
                SUM(num_esq4M)               AS total_esq4M,
                SUM(num_esq6M)               AS total_esq6M,
                SUM(num_dosaje_Hb)           AS total_dosaje_Hb
            FROM DBFED2026.dbo.IRVIN_FED_MC_02_01 {w}
