# fed_si_03_01.py
# Router para el reporte FED SI-03_01 — Gestantes adolescentes (10-19 años)
# Base de datos: DBFED2026 | Tabla: IRVIN_FED_SI_03_01
...
router = APIRouter(prefix="/fed/si0301", tags=["FED SI-03_01"])
...
# ---------------------------------------------------------------------------
# GET /fed/si0301/filtros
# ---------------------------------------------------------------------------
@router.get("/filtros", summary="Valores disponibles para filtros del reporte SI-03_01")
def get_filtros(
    ...
    try:
        anios = db.execute(text(
            "SELECT DISTINCT año FROM DBFED2026.dbo.IRVIN_FED_SI_03_01 ORDER BY año"
        )).fetchall()

        meses = db.execute(text(f"""
            SELECT DISTINCT MES, {MES_ORDER} AS orden
            FROM DBFED2026.dbo.IRVIN_FED_SI_03_01 ORDER BY orden
        """)).fetchall()

        departamentos = db.execute(text(
            "SELECT DISTINCT DEPARTAMENTO FROM DBFED2026.dbo.IRVIN_FED_SI_03_01 ORDER BY DEPARTAMENTO"
        )).fetchall()

        provincias = db.execute(text(
            "SELECT DISTINCT DEPARTAMENTO, PROVINCIA FROM DBFED2026.dbo.IRVIN_FED_SI_03_01 ORDER BY DEPARTAMENTO, PROVINCIA"
        )).fetchall()

        redes = db.execute(text(
            "SELECT DISTINCT RED FROM DBFED2026.dbo.IRVIN_FED_SI_03_01 WHERE RED IS NOT NULL ORDER BY RED"
        )).fetchall()

        microredes = db.execute(text(
            "SELECT DISTINCT RED, MICRORED FROM DBFED2026.dbo.IRVIN_FED_SI_03_01 WHERE MICRORED IS NOT NULL ORDER BY RED, MICRORED"
        )).fetchall()

        categorias = db.execute(text(
            "SELECT DISTINCT CATEGORIA FROM DBFED2026.dbo.IRVIN_FED_SI_03_01 WHERE CATEGORIA IS NOT NULL ORDER BY CATEGORIA"
        )).fetchall()
...
# ---------------------------------------------------------------------------
# GET /fed/si0301/tabla-completa  — DEPARTAMENTO → PROVINCIA → DISTRITO
# ---------------------------------------------------------------------------
...
def get_tabla_completa(
    ...
    def agg(group_cols, order_cols=None):
        ...
        return text(f"""
            ...
            FROM DBFED2026.dbo.IRVIN_FED_SI_03_01 {w}
...
# ---------------------------------------------------------------------------
# GET /fed/si0301/tabla-redes  — RED → MICRORED → ESTABLECIMIENTO
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# GET /fed/si0301/tabla-nominal  — Lista de pacientes individuales
# ---------------------------------------------------------------------------
@router.get("/tabla-nominal", summary="Lista nominal de gestantes adolescentes")
def get_tabla_nominal(
    anio: int = Query(...), mes: str = Query(...),
    departamento:    Optional[str] = Query(None),
    provincia:       Optional[str] = Query(None),
    ...
    categoria:       Optional[str] = Query(None),
    numerador:       Optional[int] = Query(None, description="Filtrar por resultado: 1=con atención, 0=sin atención"),
    ...
    if departamento:    filters.append("UPPER(DEPARTAMENTO) = UPPER(:departamento)");       params["departamento"] = departamento
    if provincia:       filters.append("UPPER(PROVINCIA) = UPPER(:provincia)");             params["provincia"] = provincia
    if distrito:        filters.append("UPPER(DISTRITO) = UPPER(:distrito)");               params["distrito"] = distrito
    if red:             filters.append("UPPER(RED) = UPPER(:red)");                         params["red"] = red
    if microred:        filters.append("UPPER(MICRORED) = UPPER(:microred)");               params["microred"] = microred
    if establecimiento: filters.append("UPPER(ESTABLECIMIENTO) = UPPER(:establecimiento)"); params["establecimiento"] = establecimiento
    if categoria:       filters.append("UPPER(CATEGORIA) = UPPER(:categoria)");             params["categoria"] = categoria
    if numerador is not None: filters.append("numerador = :numerador");                     params["numerador"] = numerador
    w = "WHERE " + " AND ".join(filters)

    try:
        total_count = db.execute(text(
            f"SELECT COUNT(*) FROM DBFED2026.dbo.IRVIN_FED_SI_03_01 {w}"
        ), params).scalar()

        rows = db.execute(text(f"""
            SELECT DEPARTAMENTO, PROVINCIA, DISTRITO, UNIDAD_EJECUTORA,
                   RED, MICRORED, ESTABLECIMIENTO, CATEGORIA,
                   año, MES, renaes, num_doc, fecha_atendida, edad,
                   denominador, numerador
            FROM DBFED2026.dbo.IRVIN_FED_SI_03_01 {w}
            ORDER BY DEPARTAMENTO, PROVINCIA, DISTRITO, ESTABLECIMIENTO
            OFFSET :skip ROWS FETCH NEXT :limit ROWS ONLY
        """), {**params, "skip": skip, "limit": limit}).fetchall()
...
# ---------------------------------------------------------------------------
# GET /fed/si0301/resumen
# ---------------------------------------------------------------------------
@router.get("/resumen", summary="Resumen de gestantes adolescentes por año y mes")
def get_resumen(
